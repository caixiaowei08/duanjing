...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="nonlocalMemberList" checkbox="true" fitColumns="false" title="来沪党员" actionUrl="nonlocalMemberController.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgCol title="批准时间"  field="memberApproveTime"  hidden="true" formatter="yyyy-MM-dd" queryMode="single"  width="120"></t:dgCol>
   ...
   <t:dgDelOpt title="删除" url="nonlocalMemberController.do?doDel&id={id}" />
   <t:dgToolBar title="录入" icon="icon-add" url="nonlocalMemberController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="nonlocalMemberController.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="nonlocalMemberController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="nonlocalMemberController.do?goUpdate" funname="detail"></t:dgToolBar>
<%--    <t:dgToolBar title="导入" icon="icon-put" funname="ImportXls"></t:dgToolBar>
 ...
 <script src = "webpage/com/dj/member/nonlocal/nonlocalMemberList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#nonlocalMemberListtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#nonlocalMemberListtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'nonlocalMemberController.do?upload', "nonlocalMemberList");
}

//导出
function ExportXls() {
	JeecgExcelExport("nonlocalMemberController.do?exportXls","nonlocalMemberList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("nonlocalMemberController.do?exportXlsByT","nonlocalMemberList");
}
 </script>