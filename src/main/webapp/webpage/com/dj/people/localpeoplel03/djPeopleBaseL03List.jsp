...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="djPeopleBaseL03List" checkbox="true" fitColumns="false" title="人在户不在" actionUrl="djPeopleBaseL03Controller.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgCol title="现居所属宅"  field="villageCurrent" query="true"   queryMode="single" dictionary="vill_type" width="160"></t:dgCol>
   ...
   <t:dgDelOpt title="删除" url="djPeopleBaseL03Controller.do?doDel&id={id}" />
   <t:dgToolBar title="录入" icon="icon-add" url="djPeopleBaseL03Controller.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="djPeopleBaseL03Controller.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="djPeopleBaseL03Controller.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="djPeopleBaseL03Controller.do?goUpdate" funname="detail"></t:dgToolBar>
<%--    <t:dgToolBar title="导入" icon="icon-put" funname="ImportXls"></t:dgToolBar>
 ...
 <script src = "webpage/com/dj/people/localpeoplel03/djPeopleBaseL03List.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#djPeopleBaseL03Listtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='birthdate_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='birthdate_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='entryTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='entryTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='dueTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL03Listtb").find("input[name='dueTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'djPeopleBaseL03Controller.do?upload', "djPeopleBaseL03List");
}

//导出
function ExportXls() {
	JeecgExcelExport("djPeopleBaseL03Controller.do?exportXls","djPeopleBaseL03List");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("djPeopleBaseL03Controller.do?exportXlsByT","djPeopleBaseL03List");
}
 </script>