...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="djPeopleBaseL02List" checkbox="true" fitColumns="false" title="户在人不在" actionUrl="djPeopleBaseL02Controller.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgCol title="现居住地区"  field="districtCurrent"  hidden="true"  queryMode="single" dictionary="dist_type" width="140"></t:dgCol>
   <t:dgCol title="现居住村"  field="townCurrent"  hidden="true"  queryMode="single" dictionary="town_type" width="140"></t:dgCol>
   <t:dgCol title="现居所属宅"  field="villageCurrent"  hidden="true"  queryMode="single" dictionary="vill_type" width="160"></t:dgCol>
   <t:dgCol title="现居门牌号"  field="ridgeNoCurrent"  hidden="true"  queryMode="single"  width="160"></t:dgCol>
   ...
   <t:dgDelOpt title="删除" url="djPeopleBaseL02Controller.do?doDel&id={id}" />
   <t:dgToolBar title="录入" icon="icon-add" url="djPeopleBaseL02Controller.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="djPeopleBaseL02Controller.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="djPeopleBaseL02Controller.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="djPeopleBaseL02Controller.do?goUpdate" funname="detail"></t:dgToolBar>
 ...
 <script src = "webpage/com/dj/people/localpeoplel02/djPeopleBaseL02List.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#djPeopleBaseL02Listtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='birthdate_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='birthdate_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='entryTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='entryTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='dueTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseL02Listtb").find("input[name='dueTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'djPeopleBaseL02Controller.do?upload', "djPeopleBaseL02List");
}

//导出
function ExportXls() {
	JeecgExcelExport("djPeopleBaseL02Controller.do?exportXls","djPeopleBaseL02List");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("djPeopleBaseL02Controller.do?exportXlsByT","djPeopleBaseL02List");
}
 </script>