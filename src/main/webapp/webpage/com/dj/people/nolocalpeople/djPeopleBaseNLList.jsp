...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="djPeopleBaseNLList" checkbox="true" fitColumns="false" title="来沪人员" actionUrl="djPeopleBaseNLController.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgDelOpt title="删除" url="djPeopleBaseNLController.do?doDel&id={id}" />
   <t:dgToolBar title="录入" icon="icon-add" url="djPeopleBaseNLController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="djPeopleBaseNLController.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="djPeopleBaseNLController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="djPeopleBaseNLController.do?goUpdate" funname="detail"></t:dgToolBar>
<%--    <t:dgToolBar title="导入" icon="icon-put" funname="ImportXls"></t:dgToolBar>
 ...
 <script src = "webpage/com/dj/people/nolocalpeople/djPeopleBaseNLList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#djPeopleBaseNLListtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='birthdate_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='birthdate_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='entryTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='entryTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='dueTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#djPeopleBaseNLListtb").find("input[name='dueTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'djPeopleBaseNLController.do?upload', "djPeopleBaseNLList");
}

//导出
function ExportXls() {
	JeecgExcelExport("djPeopleBaseNLController.do?exportXls","djPeopleBaseNLList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("djPeopleBaseNLController.do?exportXlsByT","djPeopleBaseNLList");
}
 </script>